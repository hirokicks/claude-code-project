...
この作品は普段から常に動いています（チルトの±22°スイング、レイヤーの回転、呼吸のような脈動、木目の揺らぎ、成長波、波紋）。遷移中もこれが全開だと、モーフの動きと重なって騒がしく見えます。

`calm` はその常時アニメーションを遷移中だけ弱める倍率です。

| 値 | 見え方 |
|---|---|
| `0` | 常時アニメーションが止まり、形の変化だけが見える（最も素直） |
| `0.15` | **既定値。** わずかに生気を残しつつ、モーフがはっきり読める |
| `1` | 従来通り。常時アニメーションもそのまま動き続ける |

振幅ではなく**時間の進み方**を落とす実装なので、遷移の開始・終了で位相が飛ぶことはありません。`NenrinArt.create(canvas, config, { calm: 0 })` で既定値も変えられます。
...
動きが騒がしく感じる場合は「遷移中に残す揺らぎ」を 0 に近づけてください（既定 0.15）。常時アニメーションが静まり、形の変化だけが見えるようになります。
